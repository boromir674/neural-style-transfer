...
def test_image(image_path, image_factory, test_image):
    in_memory_image = image_factory.from_disk(test_image_path := test_image(image_path))
    assert in_memory_image.file_path == test_image_path
    assert in_memory_image.matrix.shape == (300, 400, 3)
...
#     import numpy as np
#     from artificial_artwork.image import ImageProcessor, ImageFactory, ConfigMeansShapeMissmatchError
