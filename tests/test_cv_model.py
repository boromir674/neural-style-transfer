...
def test_pretrained_model(model: NSTTestingModelProtocol, graph_factory):
    ...
    if env_var == prod_env_var and not os.environ.get(prod_env_var):
        ...
        pytest.warns(UserWarning, match=f"Env var name as reported by mode handler is {env_var}, but it is not found when quering for ENV VARS. Is this intended?")
...
def test_code_builds_layers_expected_to_be_found_in_prod_image_model(
    ...
    all_conv_layers_requested_recognized = set(runtime_layers_with_weights).issubset(set(prod_vgg_layers))

    # THEN all the requested layers with weights, should be found in the original Image Model
    assert all_conv_layers_requested_recognized, f"These requsted layers where not identified: {set(DEFAULT_VGG_LAYERS_TO_BUILD) - set(prod_vgg_layers)}"
