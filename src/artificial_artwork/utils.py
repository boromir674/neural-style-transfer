...
def read_images(content, style, auto_resize_style=False):
    ...
    logger.info("Loaded CONTENT image tensor [1 x Height x Width x Channels] %s", image_manager.content_image.matrix.shape)
    logger.debug("CONTENT image Width x Height: %s x %s", image_manager.content_image.matrix.shape[2], image_manager.content_image.matrix.shape[1])
    ...
    if auto_resize_style:
        ...
        if image.size != content_image_dimensions:
            ...
            logger.info("Resizing STYLE image to match the CONTENT image: %s", json.dumps({
    ...
    logger.info("Loaded STYLE image tensor [1 x Height x Width x Channels] %s", image_manager.style_image.matrix.shape)
